...
def get_monitor_prog_pid(model, cpu_card):
    ...
    try:
        cmd = cf.get(conf_name, "anakin2_ps_cmd") % cpu_card
...
def jorcold_start_test_yolo(top_result_file, ut_yolo_path, jorcold_start_cmd, model, cpu_card):
    ...
    print "111111111111111111jorcold_start: %s" % jorcold_start
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 4:
        ...
        try:
            ...
            jorcold_start_cmd = cf.get(conf_name, "anakin2_jorcold_start_cmd") % (cpu_card, thread_size)
        except Exception as e:
        ...
        jorcold_start_cmd = cf.get("conf_yolo", "anakin2_jorcold_start_cmd") % (cpu_card, thread_size)
