...
class LoadCommon(object):
    ...
    def create_table_sql(self):
        ...
        try:
            ...
            logging.info("create the table log_monitor_%s sucess~~" % self.cpu_card)
        except Exception as exception:
            logging.error("create the table log_monitor_%s failed~~" % self.cpu_card)
    ...
    def insert_table_sql(self, time, total_num, pass_num, no_pass_num, success_ratio, no_right_shape=0, no_right_data=0, no_right_weight=0, no_right_c_and_h=0):
        ...
        """
        insert_sql = """INSERT INTO log_monitor_%s(time, total_num, pass_num, no_pass_num, success_ratio, no_right_shape, no_right_data, no_right_weight, no_right_c_and_h) VALUES ('%s', '%d', '%d', '%d', '%f', '%d', '%d', '%d', '%d')""" % (self.cpu_card, time, total_num, pass_num, no_pass_num, success_ratio, no_right_shape, no_right_data, no_right_weight, no_right_c_and_h)
        print insert_sql
        logging.info("start instert the sql")
        try:
            self.mysql.executes(insert_sql)
            logging.info("insert into log_monitor_%s success!!!" % self.cpu_card)
        except Exception as exception:
            logging.error("insert into log_monitor_%s failed!!!" % self.cpu_card)
