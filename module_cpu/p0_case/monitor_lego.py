...
def monitor_lego_prog(time_interval, top_result_file, pid):
#    # cmd1: top
...
#    os.system(cmd)
    ...
    while True:
        # check the main lego UT test is over or not
...
def jorcold_start_test_yolo(top_result_file, ut_yolo_path, jorcold_start_cmd, model, cpu_card):
    ...
    ut_lego_path = ut_yolo_path
    jorcold_start = jorcold_start_cmd
    # 1.change path to the lego ut bin path
    os.chdir(ut_lego_path)
    ...
    print pid
    monitor_lego_prog(time_interval, top_result_file, pid)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 4:
        ...
        jorcold_start_cmd = cf.get("conf_yolo", "lego_jorcold_start_cmd") % (thread_size)
    else:
