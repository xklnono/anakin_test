...
class LoadCommon(object):
    ...
    def create_table_sql_top_avg_model_paddle(self):
        """
        create the table of top_list_1sec_avg_paddle
        """
        create_table_sql = """CREATE TABLE IF NOT EXISTS top_list_1sec_paddle_avg_%s(
        ...
        logging.info("start create the table : top_list_1sec_avg_paddle")
        try:
            self.mysql.execute(create_table_sql)
            logging.info("create the table top_list_1sec_paddle_avg_%s sucess~~" % self.gpu_card)
        except Exception as exception:
            logging.error("create the table top_list_1sec_paddle_avg_% failed~~" % self.gpu_card)

    def insert_table_sql_top_avg(self, top_pertime, cpu_list_1sec, phy_mem_list_1sec, virt_mem_list_1sec):
        """
        execute the insert sql
        """
        insert_sql = """INSERT INTO top_list_1sec_paddle_avg_%s(time, model_paddle_cpu_ratio_avg, model_paddle_phy_mem_size_avg, model_paddle_vir_mem_size_avg) VALUES ('%s', '%f', '%f', '%f')""" % (self.gpu_card, top_pertime, cpu_list_1sec, phy_mem_list_1sec, virt_mem_list_1sec)
        #print insert_sql
        logging.info("start instert the sql")
        try:
            self.mysql.executes(insert_sql)
            logging.info("insert into top_list_1sec_paddle_avg_%s success!!!" % self.gpu_card)
        except Exception as exception:
            logging.error("insert into top_list_1sec_paddle_avg_%s failed!!!" % self.gpu_card)
