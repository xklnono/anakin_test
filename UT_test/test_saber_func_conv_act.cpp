...
TEST(TestSaberFuncNV, test_func_conv_relu_fusion) {
    ...
    LOG(INFO) << "|--test strategy=SPECTFY: ";
...
//!test reset_output_shape func
TEST(TestSaberFuncNV, test_func_conv_relu_reset_output_shape) {
    ...
    LOG(INFO)<<"test conv + relu + reset output shape:";
    ...
    // init assume output tensor has been reshpaed by user.
    conv.init(input, output, param, SPECIFY, VENDER_IMPL, ctx1);
    conv.init(input, output2, param, SPECIFY, VENDER_IMPL, ctx1); 
...
TEST(TestSaberFuncNV, test_func_conv_scale_relu_fusion) {
    ...
    // ! test ScaleParam vector,vector,bool,int,int 
    ...
    // ! test ScaleParam vector,false,int,int
    ScaleParam<TensorDf4> scale_param1(scale_w, false);
    ConvActiveParam<TensorDf4> param1(conv_param, active_param, scale_param1);
    ...
    conv.init(input, output, param1, SPECIFY, VENDER_IMPL, ctx1);
