...
TEST(TestSaberFuncNV, test_func_constructor) {
    ...
    act.compute_output_shape(output, input, param);
    ...
    act.init(input, output, param, RUNTIME, VENDER_IMPL, ctx1);
    act(input, output, param, ctx1);
    output_dev.sync();
    print_tensor_device(output_dev);
    cudaDeviceSynchronize();
    ...
    act.init(input, output, param, STATIC, VENDER_IMPL, ctx1);
    act(input, output, param, ctx1);
    output_dev.sync();
    print_tensor_device(output_dev);
    cudaDeviceSynchronize();
    ...
    act.init(input, output, param, UNKNOWN, VENDER_IMPL, ctx1);
    act(input, output, param, ctx1);
    output_dev.sync();
    print_tensor_device(output_dev);
    cudaDeviceSynchronize();
...
TEST(TestSaberFuncNV, test_func_reset_tensor) {
	...
	act.compute_output_shape(output, input, param);
    //test reset tensor
    act.reset_output_shape(input, output2, param, ctx1);
    ...
    //init assume output tensor has been reshpaed by user.
    act.init(input, output, param, SPECIFY, VENDER_IMPL, ctx1);
    act.init(input, output2, param, SPECIFY, VENDER_IMPL, ctx1);
    act(input, output, param, ctx1);
    act(input, output2, param, ctx1);
    
    //init assume output2 tensor has been reshpaed by user.
    output_dev2.sync();
