...
TEST(TestSaberFuncConcatNV, test_func_concat_NV) {
    ...
    ConcatParam<TensorDf4> param1(concat_axis);
    ConcatParam<TensorDf4> param(param1);
    ...
    concat_dev.compute_output_shape(output_dev_4d, input_dev_4d, param);
    ...
    LOG(INFO) << "concat initialization";
    concat_dev.init(input_dev_4d, output_dev_4d, param, STATIC, SABER_IMPL, ctx_dev);
...
//    CHECK_EQ(shape_out_v[0] == shape_out, true) << "compute output shape error";
    LOG(INFO) << "concat initialization";
    concat_dev.init(input_dev_4d, output_dev_4d, param, SPECIFY, VENDER_IMPL, ctx_dev);
...
TEST(TestSaberFuncConcatNV, test_func_concat_NV_reset_output_shape) {
    Env<NV>::env_init();
    typedef TargetWrapper<NV> API;
    typedef Tensor<NV, AK_FLOAT, NCHW> TensorDf4;
    typedef TensorDf4::Dtype dtype;
    ...
    TensorDf4 tdev_out;
    output_dev_4d.push_back(&tdev_out);
    output_dev_4d2.push_back(&tdev_out);
    LOG(INFO) << "concat compute output shape";
    concat_dev.compute_output_shape(output_dev_4d, input_dev_4d, param);
    LOG(INFO) << "output shape: " << tdev_out.valid_shape()[0] << ", " \
              << tdev_out.valid_shape()[1] << ", " << tdev_out.valid_shape()[2] \
              << ", " << tdev_out.valid_shape()[3];
    LOG(INFO) << "concat compute output2 shape";
    concat_dev.reset_output_shape(input_dev_4d,output_dev_4d2, param,ctx_dev);
    LOG(INFO) << "output shape: " << tdev_out.valid_shape()[0] << ", " \
              << tdev_out.valid_shape()[1] << ", " << tdev_out.valid_shape()[2] \
              << ", " << tdev_out.valid_shape()[3];                                            
    tdev_out.re_alloc(shape_out);
    LOG(INFO) << "concat initialization";
    concat_dev.init(input_dev_4d, output_dev_4d, param, SPECIFY, SABER_IMPL, ctx_dev);
    concat_dev.init(input_dev_4d, output_dev_4d2, param, SPECIFY, SABER_IMPL, ctx_dev);
    LOG(INFO) << "concat compute";
    SaberTimer<NV> t1;
    t1.clear();
    t1.start(ctx_dev);
    ...
    t2.start(ctx_dev);
    for (int i = 0; i < test_iter; ++i) {
        concat_dev(input_dev_4d, output_dev_4d2, param, ctx_dev);
        output_dev_4d2[0]->sync();
    }
    t2.end(ctx_dev);
    float ts2 = t2.get_average_ms();
    printf("total time : %.4f, avg time : %.4f\n", ts2, ts2 / test_iter);
    CUDA_POST_KERNEL_CHECK;
    print_tensor_device(*output_dev_4d[0]);
    cudaDeviceSynchronize();
