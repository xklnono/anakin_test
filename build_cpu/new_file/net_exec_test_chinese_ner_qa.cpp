...
DEFINE_GLOBAL(std::string, model_dir, "/home/qa_work/CI/workspace/sys_anakin_compare_output/chinese_ner/models/");
DEFINE_GLOBAL(std::string, input_file, "/home/qa_work/CI/workspace/sys_anakin_compare_output/chinese_ner/input_file/data_file");
DEFINE_GLOBAL(std::string, output_file, "/home/qa_work/CI/workspace/sys_anakin_compare_output/chinese_ner/images_output/");
DEFINE_GLOBAL(std::string, time_result, "/home/qa_work/CI/workspace/sys_anakin_compare_output/chinese_ner/time/");
...
//        timer.end(ctx);
    ...
    //fp << "summary_thread :thread total : " << use_ms << " ms, avg = " << (use_ms/data_in->size()/GLB_batch_size) <<std::endl;
    //fp.close();
    //get out
    ...
    for(int i = 0; i < graph_outs.size(); i++) {
        ...
        for(int j=0; j< tensor_out_i_p->size(); j++) {
            printf("%f \n", tensor_out_i_p->data()[j]);
        }   
    }
    free(graph);
}
...
int main(int argc, const char** argv){
    ...
    if (argc < 1) {
        ...
        ./output/unit_test/net_exec_test_chinese_ner \
            thread_num \n";
        exit(0);
    } else if (argc >= 2) {
