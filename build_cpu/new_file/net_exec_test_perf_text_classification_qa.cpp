...
DEFINE_GLOBAL(std::string, model_dir, "/home/qa_work/CI/workspace/sys_anakin_compare_output/text_classification/models/");
DEFINE_GLOBAL(std::string, input_file, "/home/qa_work/CI/workspace/sys_anakin_compare_output/text_classification/input_file/out.ids.txt");
DEFINE_GLOBAL(std::string, output_file, "/home/qa_work/CI/workspace/sys_anakin_compare_output/text_classification/images_output/");
DEFINE_GLOBAL(std::string, time_result, "/home/qa_work/CI/workspace/sys_anakin_compare_output/text_classification/time/");
...
//    Context<X86> ctx;
    ...
    //get ms
    std::ofstream fp;
    fp.open(GLB_time_result + "Anakin2_time" + ".txt", std::ios::app);
    fp << "summary_thread :thread total : " << use_ms << " ms, avg = " << (use_ms/data_in->size())<<", consume words = "<<word_sum <<std::endl;
    fp.close();
    //get out 
    //char *file = (GLB_output_file + "anakin.txt").c_str() ;
    //    for(int i = 0; i < graph_outs.size(); i++) {
    //        LOG(INFO) << "graph outs: " << graph_outs[i];
    //        auto tensor_out_i_p = net_executer.get_out(graph_outs[i]);
    //        record_dev_tensorfile(tensor_out_i_p, file);
    //    } 
    free(graph);
}
#define ONE_THREAD 1
...
//                new std::thread(&anakin_net_thread, &host_tensor_p_in_list[i]),models[0]);
    ...
    fp.open(GLB_time_result + "Anakin2_time" + ".txt" , std::ios::app);
    fp << "summary: "<<"thread num = "<< thread_num <<",total time = " << use_ms << "ms ,batch = "<< batch_num
             <<",word sum = "<<GLB_word_count<<", seconde/line = "<<(use_ms/word_idx.size())
             <<",QPS = "<<(word_idx.size()/use_ms*1000); 
    fp.close();
}
...
int main(int argc, const char** argv) {
    ...
    if (argc < 1) {
        ...
        ./output/unit_test/net_exec_test_text_classification \n \
             thread_num\n";
        exit(0);
    } else if (argc >=2 ) {
        GLB_run_threads = atoi(argv[1]);
    }
    if (argc >=3 ) {
        GLB_run_threads = atoi(argv[1]);
        GLB_batch_size = atoi(argv[2]);
    }
    if (argc >= 4) {
        GLB_split_index = atoi(argv[3]);
    }
    if (argc >= 5) {
        GLB_split_word = std::string(argv[4]);
    }
