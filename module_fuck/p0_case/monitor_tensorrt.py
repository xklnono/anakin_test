...
def get_monitor_prog_pid(model, gpu_card):
    ...
    try:
        cmd = cf.get(conf_name, "p0_tensorrt_ps_cmd") % gpu_card
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 4:
        ...
        try:
            ...
            jorcold_start_cmd = cf.get(conf_name, "p0_tensorrt_jorcold_start_cmd") % (gpu_card, batch_size)
        except Exception as e:
            print ("\033[0;31;m[error]: Pls Check The Modle input wrong!\033[0m")
            sys.exit(1)
    if len(sys.argv) == 3:
        ...
        ut_yolo_path = "/home/qa_work/CI/workspace/sys_tensorRT_merge_build/output"
        jorcold_start_cmd = cf.get("conf_yolo", "p0_tensorrt_jorcold_start_cmd") % (gpu_card, batch_size)
