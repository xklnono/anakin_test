...
def get_monitor_prog_pid(model, gpu_card):
    ...
    try:
        cmd = cf.get(conf_name, "p1_anakin2_ps_cmd") % gpu_card
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 4:
        ...
        try:
            ...
            jorcold_start_cmd = cf.get(conf_name, "p1_tensorrt_jorcold_start_cmd") % (gpu_card, batch_size)
        except Exception as e:
            print ("\033[0;31;m[error]: Pls Check The Modle input wrong!\033[0m")
            sys.exit(1)
    else:
           print "usage:"
           print "    python monitor_anakin.py model(name) batch_size(int) gpu_card(p4/k1200)"
