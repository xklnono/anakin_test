...
class LoadCommon(object):
    ...
    def select_accuracy_rate(self, gpu_card):
        """
        execute the select sql
        """
        select_sql = """SELECT success_ratio from log_monitor_%s order by time desc limit 1;""" % gpu_card

        try:
            # this is for select result is tulple to string
            select_result = self.mysql.executes(select_sql)[0][0]
            logging.info("select log_monitor_%s success!!!" % gpu_card)
            return select_result
        except Exception as exception:
            logging.error("select log_monitor_%s failed!!!" % gpu_card)
